...
def hex_view(hex_object_id):
    ...
    if queried_hex_object.is_private and (current_user.is_anonymous or current_user.id != queried_hex_object.owner_id):
        ...
        next_hex = hex_object.HexObject.query.filter(hex_object.HexObject.owner_id!=app.config['ANON_USER_ID'], hex_object.HexObject.is_private == False).order_by(func.random()).limit(1).first()
...
def form_user_create():
    ...
    if request.form and create_user.validate_on_submit() and request.form['password'] == request.form['password_two'] and not get_user_by_name(request.form['username']):
        ...
        flash('there was an error in account creation, try again with a different username')
